    ...
    <>
      <motion.nav
        ...
        className={`fixed top-0 left-0 right-0 z-50 flex items-center justify-between backdrop-blur-xl transition-all duration-400 ${
          scrolled
            ? "py-2 px-7 bg-white/90 border-b border-[var(--border)]"
            : "py-3.5 px-7 bg-white/90 "
        }`}
      >
        ...
        <div className="hidden lg:flex items-center gap-4">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.label}
              href={link.href}
              className="text-brand-tertiary/80 hover:text-brand-primary transition-colors font-body text-xs font-medium tracking-wide uppercase "
            >
              {link.label}
            </Link>
          ))}
        ...
        <button
          ...
          className="lg:hidden text-brand-tertiary p-2"
          aria-label="Menü"
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-40 bg-white text-brand-tertiary pt-24 px-8 lg:hidden"
          >
              ...
              {NAV_LINKS.map((link, i) => (
                ...
                >
                  <Link
                    ...
                    className="text-brand-tertiary font-display text-2xl font-bold "
                  >
                    {link.label}
                  </Link>
                </motion.div>
